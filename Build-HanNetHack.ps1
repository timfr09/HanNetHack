...
.DESCRIPTION
  저장소 루트에서 실행하세요. Lua / PDCursesMod 가 없으면 PowerShell(Invoke-WebRequest + tar)로 받습니다.
  빌드 후 기본으로 sys\windows\install.cmd 를 실행해 install\HanNetHack\ 에 복사합니다.

.EXAMPLE
  .\Build-HanNetHack.ps1

.EXAMPLE
  .\Build-HanNetHack.ps1 -CompilePo -Target Build

.EXAMPLE
  .\Build-HanNetHack.ps1 -SkipFetch -SkipInstall
#>
[CmdletBinding()]
param(
