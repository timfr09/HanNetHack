...
setuwep(struct obj *obj)
{
    ...
    if (uwep == obj && artifact_light(olduwep) && olduwep->lamplit) {
        ...
        if (!Blind)
            pline(_("%s shining."), Tobjnam(olduwep, C_("light", "stop")));
    }
    if (uwep == obj
...
ready_weapon(struct obj *wep)
{
    ...
    if (!wep) {
        ...
        if (artifact_light(wep) && !wep->lamplit) {
            ...
            if (!Blind)
                pline(_("%s to shine %s!"), Tobjnam(wep, C_("light", "begin")),
        ...
        if (wep->unpaid) {
            ...
            if ((this_shkp = shop_keeper(inside_shop(u.ux, u.uy)))
                ...
                if (!Deaf && this_shkp->data->msound > MS_ANIMAL)
                    pline(_("%s %s \"You be careful with my %s!\""),
                          shkname(this_shkp), _(says()), xname(wep));
                else
                    pline(_("%s looks apprehensive about your wielding %s %s."),
...
uwepgone(void)
{
    if (uwep) {
        if (artifact_light(uwep) && uwep->lamplit) {
            end_burn(uwep, FALSE);
            if (!Blind)
                pline(_("%s shining."), Tobjnam(uwep, C_("light", "stop")));
        }
