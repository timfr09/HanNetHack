...
### 1.3 번역 원칙
- 간결하고 명확한 표현
- 기본은 지문·내러티브에 '-다'체 (§4.2 참고: 프롬프트·시스템 안내 등은 존댓말)
- 고유명사는 음역 또는 의역 선택
- 게임 용어 일관성 유지
...
2. 게임 속 일이 일어났다고 서술하는가? (전투·상태·지문) → '-다'체 (…다/…었다. 해요·합니다 금지).
3. 말을 거는 쪽이 상점 주인인가? → 반존대(-요) (과한 격식 “합니다”, 과한 “십시오”는 피함).
4. 일반 몬스터·NPC 대화인가? → 캐주얼 반말 (…해./…가라.).
5. 플레이어에게 거의 안 보이는 내부 오류·불가능 메시지인가? → '-다'체로 짧게; 플레이어를 부르지 않음.
