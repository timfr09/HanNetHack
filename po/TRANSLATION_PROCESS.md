...
번역 **파일**은 이미 `ko_manual.po` / `dat/locale/ko/`로 분리돼 있다. **커밋이 섞여도 런타임 문제는 거의 없고**, 불편은 리뷰·bisect·체리픽뿐이다. 앞으로:

| | 머지 PR | 번역 PR |
|--|---------|---------|
| 범위 | 충돌·래핑·빌드 + **최소 새 문자열 번역** | 문체 다듬기·품질 수정 (`i18n(ko):` / `fix(i18n/ko):`) |
| 금지 | 대량 polish를 머지에 섞기 | 업스트림 머지 충돌 해결과 한 PR에 묶기 |

상세·추적 명령은 [doc/i18n-upstream-merge.md](../doc/i18n-upstream-merge.md)「머지 PR vs 번역 PR」.
