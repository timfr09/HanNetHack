...
조사 마커 `{은/는}`, `{이/가}`, `{을/를}` 등과 문체·용어 통일·포맷 문자열 규칙은 모두 **[TRANSLATION_GUIDE_KO.md](TRANSLATION_GUIDE_KO.md)** 에 정리되어 있습니다. **플레이어에게 직접 건네는 안내·질문·팁은 존댓말**, 지문·전투 묘사 등 내러티브는 `-다`체입니다. 이 README에서는 워크플로우만 다룹니다.
