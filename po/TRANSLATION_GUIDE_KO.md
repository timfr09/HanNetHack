...
### 4.5 영문 단어 뒤 조사 판정 규칙 (letter_batchim)

번역문이 `%s{이/가}`처럼 조사 마커를 달고 있을 때, `%s`가 한글이 아니라 영문 단어/약어(예: `Orc`, `Troll`, `USB`)라면 런타임이 영문 단어의 마지막 글자를 기준으로 받침을 판정합니다. 판정 기준은 **알파벳 이름의 한국어 발음**입니다 (`ko_postpos.c`의 `letter_batchim`).

| 마지막 글자 | 판정 | 이유 |
|---|---|---|
| F / L / M / N / R / S / X | 받침 있음 | 에프, 엘, 엠, 엔, 아르, 에스, 엑스 |
| 숫자 (8) | 받침 있음 | 팔 |
| 숫자 (1/3/6/7) | 받침 있음 | 일/삼/육/칠 |
| 나머지 글자/숫자 | 받침 없음 | |

**주의**: 이는 휴리스틱이므로 외래어 단어의 실제 한국어 발음과 어긋날 수 있습니다.
- `USB가` / `DNA가` / `HTML이` → 약어는 알파벳 이름 그대로 읽으므로 정확.
- `Elf이` → `f` 마지막 글자로는 받침 있음. 그러나 "엘프"는 받침 없음 → `Elf가`가 자연스러움. **오류 케이스**.

이런 오류 케이스는 번역 카탈로그에서 직접 override하세요:
```po
msgid "The elf hits you."
msgstr "엘프가 당신을 때렸다."
```

(원문 `%s`를 해소된 한국어 단어로 대체하거나, 조사 마커 대신 적절한 조사를 이미 결정된 상태로 박아 넣는 방식.)
...
> **⚠️ LLM 에이전트 주의사항**: 번역 수정 시 **반드시 ko_manual.po**를 편집하세요!
> ko.po를 직접 수정하면 `make update-po` 실행 시 덮어씌워져 모든 작업이 손실됩니다.
> ko_manual.po에 추가한 번역은 병합 시 ko.po보다 우선 적용됩니다.
...
`msgcat --use-first`는 빈 문자열(`msgstr ""`)을 "미번역"으로 간주하여 무시합니다.
따라서 ko_manual.po에서 빈 문자열로 덮어쓰기가 불가능합니다.

**빈 문자열이 필요한 경우:**
1. ko.po를 직접 수정 (update-po 시 주의 필요)
2. 또는 조합 메시지의 다른 부분에서 의미를 완성하도록 번역

예: 동사 접두어 `"are "`, `"were "`는 ko.po에서 이미 빈 문자열로 되어 있음.
...
- **조합 메시지 어순 수정** (from_what() 관련 등)
- **ko.po의 fuzzy/잘못된 번역 덮어쓰기**
