# HanNetHack 번역 작업 완벽 가이드

## 목차
1. [프로젝트 개요](#1-프로젝트-개요)
2. [프로젝트 구조](#2-프로젝트-구조)
3. [번역 시스템 (gettext)](#3-번역-시스템-gettext)
4. [한국어 번역 규칙](#4-한국어-번역-규칙)
5. [소스 코드에서 문자열 래핑](#5-소스-코드에서-문자열-래핑)
6. [번역 파일 관리](#6-번역-파일-관리)
7. [조합 메시지 패턴 번역](#7-조합-메시지-패턴-번역)
8. [from_what() 원인 표시 처리](#8-from_what-원인-표시-처리)
9. [동사 접두어 상수 처리](#9-동사-접두어-상수-처리)
10. [주석 작성 규칙](#10-주석-작성-규칙)
11. [특수 번역 항목](#11-특수-번역-항목)
12. [도움말 및 데이터 파일 번역](#12-도움말-및-데이터-파일-번역)
13. [빌드 및 검증](#13-빌드-및-검증)
14. [테스트 방법](#14-테스트-방법)
15. [문제 해결](#15-문제-해결)
16. [전체 작업 흐름](#16-전체-작업-흐름)
17. [유용한 명령어 모음](#17-유용한-명령어-모음)

---

## 1. 프로젝트 개요

### 1.1 HanNetHack이란?
NetHack 3.7의 한국어 번역 프로젝트입니다. GNU gettext 기반 i18n 시스템을 사용합니다.

### 1.2 번역 범위
- **소스 코드 메시지**: 게임 내 모든 텍스트 (약 9,000+ 문자열)
- **도움말 파일**: help, cmdhelp, opthelp 등
- **데이터 파일**: rumors, epitaph, engrave 등
- **Lua 스크립트**: 퀘스트 대화, 던전 메시지 등

### 1.3 번역 원칙
- 간결하고 명확한 표현
- 기본은 지문·내러티브에 '-다'체 (§4.2 참고)
- **플레이어에게 직접 건네는 말**(확인 질문·팁·단축키 안내·복구 안내 등)은 **존댓말**(합니다/하세요/하시겠습니까 계열). 지문·전투 묘사만 '-다'체.
- 고유명사는 음역 또는 의역 선택
- 게임 용어 일관성 유지
- **핵심 목표는 '소설스럽게 읽히는 문장'**이다. 단, 장식적 표현을 덧붙이기보다 문장 결·호흡을 다듬는 방식으로 구현한다.
- **원문 의미 보존 우선**: 원문에 없는 정보(장소·거리·감정·원인)를 임의로 추가하지 않는다.
- **2인칭 호칭은 문맥 기반**으로 선택한다. 시스템 안내·중립 지문은 '당신', 위협/조롱/캐릭터 대사는 '너/네'를 허용한다.

---

## 2. 프로젝트 구조

```
./
├── src/                          # C 소스 코드
│   ├── insight.c                 # enlightenment 메시지 (주요)
│   ├── pline.c                   # 메시지 출력
│   ├── dog.c                     # 펫 관련 (펫 이름 등)
│   ├── objnam.c                  # 아이템 이름
│   ├── monst.c                   # 몬스터 정의
│   ├── i18n.c                    # 국제화 함수
│   ├── ko_postpos.c              # 한국어 조사 처리
│   └── obj_descr_i18n.c          # 아이템 설명 번역
├── include/
│   ├── i18n.h                    # _(), N_() 매크로
│   └── ko_postpos.h              # 조사 처리 헤더
├── po/
│   ├── ko_manual.po              # 한국어 번역 (편집 대상, 저장소 커밋)
│   ├── ko.po                     # 로컬 캐시 (update-po가 생성, gitignored)
│   ├── nethack.pot               # 번역 템플릿 (자동 생성)
│   ├── messages.mo               # 컴파일된 번역
│   ├── Makefile                  # 번역 빌드
│   └── TRANSLATION_GUIDE.md      # 이 가이드
├── dat/
│   ├── locale/ko/                # 한국어 데이터 파일
│   │   ├── help                  # 도움말
│   │   ├── cmdhelp               # 명령어 도움말
│   │   ├── rumors.tru            # 진짜 소문
│   │   ├── rumors.fal            # 가짜 소문
│   │   └── *.lua                 # Lua 스크립트
│   └── nhdat                     # 컴파일된 게임 데이터
└── win/
    ├── tty/                      # TTY 인터페이스
    └── curses/                   # Curses 인터페이스
```

---

## 3. 번역 시스템 (gettext)

### 3.1 작동 원리

```
소스 코드 (.c)  →  POT 파일  →  PO 파일  →  MO 파일  →  게임
   _("text")      (템플릿)      (번역)      (바이너리)
```

### 3.2 파일 역할

| 파일 | 역할 | 편집 |
|------|------|------|
| `*.c` | 소스 코드, _() 매크로로 래핑 | O |
| `nethack.pot` | 추출된 문자열 템플릿 | X |
| `ko.po` | 한국어 번역 | O |
| `messages.mo` | 컴파일된 바이너리 | X |

### 3.3 매크로 종류

```c
_("text")              // 런타임 번역 (일반적)
N_("text")             // 마킹만 (정적 배열용)
C_("ctx", "text")      // 컨텍스트 분리 번역 (동일 문자열, 다른 번역)
gettext("text")        // _()와 동일
```

---

## 4. 한국어 번역 규칙

### 4.1 조사 처리 시스템

한국어는 앞 글자의 받침에 따라 조사가 달라집니다.
`ko_postpos.c`에서 자동 처리됩니다.

```c
// 사용 예
pline(_("%s{이/가} 공격한다."), mon_nam(mtmp));
```

#### 지원되는 조사 패턴

| 패턴 | 받침 있음 | 받침 없음 | 용도 |
|------|----------|----------|------|
| `{은/는}` | 은 | 는 | 주제 |
| `{이/가}` | 이 | 가 | 주격 |
| `{을/를}` | 을 | 를 | 목적격 |
| `{과/와}` | 과 | 와 | 접속 |
| `{으로/로}` | 으로 | 로 | 방향/도구 |
| `{아/야}` | 아 | 야 | 호격 |
| `{이다/다}` | 이다 | 다 | 서술격 |
| `{이/}` | 이 | (없음) | 주격 (생략) |

#### 예시

```
msgid "You hit %s."
msgstr "%s{을/를} 때렸다."

msgid "%s attacks!"
msgstr "%s{이/가} 공격한다!"

msgid "with %s"
msgstr "%s{으로/로}"
```

### 4.2 문체 규칙

#### 존댓말 vs 반말 구분

| 메시지 유형 | 문체 | 예시 |
|------------|------|------|
| **사용자 질문/프롬프트** | 존댓말 (-ㅂ니까/-세요) | "무엇을 버리겠습니까?", "선택하세요" |
| **팁·단축키 안내·복구 확인 등 플레이어 대면 안내** | 존댓말 | "…해 보세요.", "…하시겠습니까?", "…주세요." |
| **게임 상황 설명** | 반말 (-다) | "배가 고프다.", "%s{을/를} 때렸다." |
| **상점 주인 대화** | 반존대 (-요) | "나가기 전에 계산해 주세요.", "환영해요!" |
| **일반 NPC 대화** | 반말 (캐주얼) | "안녕.", "따라와." |
| **시스템 안내** | 존댓말 | "버그는 <%s>로 보고해 주세요." |
| **내부 점검·`impossible()`·저널/진단용 시스템 메시지** | **번역하지 않음** | `msgstr ""` 로 두어 **영어 msgid가 그대로** 나가게 한다. 플레이어가 통상 플레이에서 거의 보지 않는 오류·무결성 메시지를 한글로 채우면 디버깅·업스트림 diff·용어 일치가 어려워진다. |
| **튜토리얼·첫 화면·안내 pline** | 시스템 안내에 가깝다면 존댓말 | "튜토리얼에 진입합니다." / "…할 수 있습니다" |

#### 빠른 판별 (한 줄)

1. 플레이어에게 입력·선택·확인(y/n)을 요구하는가? (프롬프트·메뉴·getlin) → 존댓말 (ㅂ니다/ㅂ니까/하세요 계열).
2. 게임이 플레이어에게 **조작법·팁·주의**를 알려 주는가? (`nh.text` 도움창, cmdassist, 복구 대화 등) → 존댓말 (안내·권유형은 **하세요/합니다**, 질문형은 **하시겠습니까**).
3. 게임 속 일이 일어났다고 서술하는가? (전투·상태·지문) → '-다'체 (…다/…었다. 해요·합니다 금지).
4. 말을 거는 쪽이 상점 주인인가? → 반존대(-요) (과한 격식 “합니다”, 과한 “십시오”는 피함).
5. 일반 몬스터·NPC 대화인가? → 캐주얼 반말 (…해./…가라.).
6. 플레이어에게 거의 안 보이는 내부 오류·진단·`impossible()`류인가? → **번역하지 않음** (`msgstr ""` → 영어 원문 표시). 예외를 두려면 소스 맥락을 확인할 것.

같은 영어 문장이라도 `pline`(지문) vs `yn_function`(질문) vs 상점 대사로 쓰이면 문체가 달라질 수 있으므로, **원문이 호출되는 위치**(소스·맥락)를 본다.

#### 사용자 질문 (존댓말)
플레이어에게 선택이나 입력을 요청하는 프롬프트는 존댓말을 사용합니다.

```
# 좋은 예 (사용자 질문)
"무엇을 버리겠습니까?"
"어디로 이동하시겠습니까?"
"정말로 종료하시겠습니까?"

# 나쁜 예 (사용자 질문)
"무엇을 버리겠는가?"  # 너무 딱딱함
"뭘 버릴래?"          # 너무 캐주얼
```

#### 게임 상황 설명 (반말)
게임 내 상황을 설명하는 메시지는 '-다' 체를 사용합니다.

```
# 좋은 예 (게임 상황)
"배가 고프다."
"%s{을/를} 때렸다."
"문이 열렸다."

# 나쁜 예 (게임 상황)
"배가 고파요."        # 해요체 X
"%s를 때렸습니다."    # 합니다체 X
```

#### 상점 주인 대화 (반존대)
상점 주인이 손님(플레이어)에게 하는 말은 반존대(-요)를 사용합니다.

```
# 좋은 예 (상점 주인)
"나가기 전에 계산해 주세요."
"환영해요!"
"훔치려고 하지 마세요."

# 나쁜 예 (상점 주인)
"나가기 전에 계산해 줘."    # 너무 반말
"환영합니다."              # 너무 격식체
```

#### 일반 NPC 대화 (반말/캐주얼)
일반 NPC가 플레이어에게 하는 말은 캐주얼한 반말을 사용합니다.

```
# 좋은 예 (일반 NPC 대화)
"안녕."
"따라와."
"그건 취급 안 해."

# 나쁜 예 (일반 NPC 대화)
"안녕하세요."         # 너무 공손
"따라오십시오."       # 너무 격식체
```

- **간결함**: 불필요한 조사/어미 생략 가능
- **일관성**: 같은 유형의 메시지는 같은 문체

#### 기존 번역을 다듬을 때의 원칙 (품질 편집)

신규 번역과 동일하게 문체·조사·용어 규칙을 따르지만, **이미 있는 번역을 고칠 때**는 다음을 추가로 지킨다.

1. **단순 치환 금지** — 동일한 영단어·구문이 여러 `msgid`에 반복되더라도, **전역 치환**(편집기 일괄 바꾸기만으로 일괄 수정)은 피한다. 호출 맥락마다 자연스러운 표현이 다를 수 있다. 바꿀 때는 **항목별로** 소스 용례를 확인한다.
2. **게임 출력이 최종 심사** — PO 편집기 안에서 읽히는 문장보다, **실행 화면 메시지 줄**에서 읽히는 문장이 우선이다. 다듬은 뒤에는 가능하면 해당 동작을 재현해 한 줄로 읽어 본다.
3. **호출 위치 확인** — 같은 문자열이라도 `pline`(지문)·메뉴·`yn`·조합 메시지 조각으로 쓰이면 어색할 수 있다. `rg` 등으로 소스에서 용례를 찾는다.
4. **형식 보존** — `msgfmt`용 `%s`/`%d` 개수, `%n$s` 위치, 조사 API 마커(`{을/를}` 등)를 바꾸지 않는다. 바꾼 뒤에는 반드시 `make translation-ci`로 검증한다.
5. **길이·줄바꿈** — 터미널 폭이 좁을 때 메시지가 두 줄 이상으로 깨지거나 중간이 잘리면, 짧은 동의어나 어순 조정을 검토한다.
6. **회귀 의식** — 한 메시지 수정이 같은 `msgid`를 쓰는 다른 상황까지 바꾼다는 점을 기억하고, 필요하면 여러 시나리오를 확인한다.

**조합 메시지**를 다룰 때는 [§7.4](#74-조합-메시지-작업-체크리스트-필수)와 [TRANSLATION_PROCESS.md](TRANSLATION_PROCESS.md) §2.D(조합 추가 표)를 반드시 본다. 일반 다듬기 절차만으로는 부족할 수 있다.

### 4.3 용어 통일

| 영어 | 한국어 |
|------|--------|
| hit | 때리다 |
| kill | 죽이다 |
| miss | 빗나가다 |
| damage | 피해 |
| gold | 금화 |
| experience | 경험치 |
| level | 레벨 |
| dungeon | 던전 |
| monster | 몬스터 |
| armor | 갑옷 |
| weapon | 무기 |
| potion | 물약 |
| scroll | 두루마리 |
| wand | 마법봉 |
| ring | 반지 |
| amulet | 부적 |

### 4.4 고유명사 처리

#### 음역 (소리 그대로)
```
Yendor → 옌더
Gehennom → 게헨놈
Moloch → 몰록
Medusa → 메두사
```

#### 의역 (뜻 번역)
```
Amulet of Yendor → 옌더의 부적
Bell of Opening → 개봉의 종
Book of the Dead → 죽음의 책
```

### 4.5 영문 단어 뒤 조사 판정 규칙 (letter_batchim)

번역문이 `%s{이/가}`처럼 조사 마커를 달고 있을 때, `%s`가 한글이 아니라 영문 단어/약어(예: `Orc`, `Troll`, `USB`)라면 런타임이 영문 단어의 마지막 글자를 기준으로 받침을 판정합니다. 판정 기준은 **알파벳 이름의 한국어 발음**입니다 (`ko_postpos.c`의 `letter_batchim`).

| 마지막 글자 | 판정 | 이유 |
|---|---|---|
| F / L / M / N / R / S / X | 받침 있음 | 에프, 엘, 엠, 엔, 아르, 에스, 엑스 |
| 숫자 (8) | 받침 있음 | 팔 |
| 숫자 (1/3/6/7) | 받침 있음 | 일/삼/육/칠 |
| 나머지 글자/숫자 | 받침 없음 | |

**주의**: 이는 휴리스틱이므로 외래어 단어의 실제 한국어 발음과 어긋날 수 있습니다.
- `USB가` / `DNA가` / `HTML이` → 약어는 알파벳 이름 그대로 읽으므로 정확.
- `Elf이` → `f` 마지막 글자로는 받침 있음. 그러나 "엘프"는 받침 없음 → `Elf가`가 자연스러움. **오류 케이스**.

이런 오류 케이스는 번역 카탈로그에서 직접 override하세요:
```po
msgid "The elf hits you."
msgstr "엘프가 당신을 때렸다."
```

(원문 `%s`를 해소된 한국어 단어로 대체하거나, 조사 마커 대신 적절한 조사를 이미 결정된 상태로 박아 넣는 방식.)

### 4.6 만화체·폭발 의성어 (KAABLAMM / KABOOM 등)

영어 `KAABLAMM`·`Kaablamm`·`KABOOM`·`KADOOM`·`BOOM`은 **한국어 의성어로 현지화**한다. 강도는 `!` 개수로 구분하고, **종류별 표기를 통일**한다.

| 계열 (msgid 예) | 한국어 | 쓰임 |
|-----------------|--------|------|
| `KAABLAMM` / `Kaablamm` | **콰광** | 지뢰·대형 폭발. `!!!`는 가까이·강함, `Kaablamm!`는 멀리서 들림 |
| `KABOOM` | **펑** 또는 **콰광** | 부비트랩 `펑!!`, 문 폭발 `콰광!!` |
| `BOOM!` | **펑!** | 물약 연쇄 폭발 접두 등 소형 |
| `KADOOM` | **쿵!** | 바위 낙하·굴림 |
| `Whammm` / 문·왕좌 파괴 | **쾅** | 날카로운 타격·파괴(폭발과 구분) |
| `Kaboom!!! Boom!!` (천둥) | **콰광!!!** + **쾅!!** | 첫 낙뢰 + 울림 |
| `ZOT` (만화 의성어) | **맥락별** (아래) | `*ZOT*`·`쫑`으로 일괄 통일하지 않음 |

**`ZOT` 맥락별 한국어**

| 상황 | 권장 의성어 | HanNetHack |
|------|-------------|------------|
| 번개·전기·레이저·날카로운 에너지 | **찌릿!** / 파앗! / 번쩍! | `a loud ZOT!` (`sounds.c` 신탁 소리) → `큰 찌릿! 소리가 들린다!` |
| 순간 소멸·변환 | **뿅!** / 휙! | (현재 msgid 없음 — 추가 시 소스 맥락 확인) |
| 갑작스러운 타격 | **퍽!** / 팍! | (현재 msgid 없음) |

같은 `ZOT`이라도 용도가 다르면 `msgctxt`로 분리한다. `*SPLASH*`→`*첨벙*`처럼 고정 치환이 맞는 의성어도 있으나 `ZOT`은 위 표를 우선한다.

`%s!  %s %s land mine.`처럼 **의성어가 `%s`로 끼는 조합**은 `KAABLAMM!!` msgstr(`콰광!!`)이 그대로 들어가는지 시뮬레이션한다. 조합문 절차는 §7.4.

---

## 5. 소스 코드에서 문자열 래핑

### 5.1 기본 래핑

```c
// 변경 전
pline("You feel hungry.");

// 변경 후
pline(_("You feel hungry."));
```

### 5.2 포맷 문자열

```c
// 변경 전
pline("You hit %s.", mon_nam(mtmp));

// 변경 후
pline(_("You hit %s."), mon_nam(mtmp));
```

### 5.3 정적 배열 (N_() 사용)

```c
// 변경 전
static const char *messages[] = { "opt1", "opt2" };

// 변경 후
static const char *messages[] = { N_("opt1"), N_("opt2") };
// 사용: pline("%s", _(messages[i]));
```

### 5.4 조건부 문자열

```c
// 변경 전
msg = cond ? "yes" : "no";

// 변경 후
msg = cond ? _("yes") : _("no");
```

### 5.5 enl_msg() 패턴

```c
// 변경 전
enl_msg(You_, "would fly", "would have flown", " if trapped", "");

// 변경 후
enl_msg(You_, _("would fly"), _("would have flown"), _(" if trapped"), "");
```

### 5.6 래핑하면 안 되는 것

```c
// 이미 번역된 문자열
pline("%s", translated_string);

// 디버그 메시지
impossible("debug message");

// 빈 문자열
enl_msg(You_, verb, verb, suffix, "");  // 마지막 ""
```

### 5.7 i18n.h 포함 확인

```c
#include "hack.h"
#include "i18n.h"  // 필수!
```

### 5.8 C_() 컨텍스트 분리

동일한 영어 문자열이 여러 곳에서 다른 의미로 사용될 때, `C_()` 매크로로 컨텍스트를 분리합니다.

#### 문제 상황

```c
// insight.c: enlightenment 화면에서 문장으로 사용
you_are(_("invisible"), from_what(INVIS));
// → "당신은 투명 상태이다 (투명의 반지 때문에)."

// timeout.c: propertynames[] 배열에서 레이블로 사용
N_("invisible")
// → "투명" (목록 레이블)
```

`_("invisible")`를 "투명 상태이다"로 번역하면 timeout.c에서 "투명 상태이다"가 출력됨.
`_("invisible")`를 "투명"으로 번역하면 insight.c에서 "당신은 투명."이 되어 서술어가 빠짐.

#### 해결: C_() 사용

```c
// insight.c - 컨텍스트 분리
you_are(C_("insight", "invisible"), from_what(INVIS));

// timeout.c - 기존 _() 유지
N_("invisible")
```

#### ko_manual.po 항목

```
# timeout.c 레이블용 (기존)
msgid "invisible"
msgstr "투명"

# insight.c 문장용 (새로 추가)
msgctxt "insight"
msgid "invisible"
msgstr "투명 상태이다"
```

#### 현재 C_("insight") 적용 항목

| 영어 msgid | insight.c (문장) | 기본 (레이블) |
|-----------|-----------------|-------------|
| `levitating` | 부유 상태이다 | 부유 |
| `flying` | 비행 상태이다 | 비행 |
| `swimming` | 수영 중이다 | 수영 |
| `stunned` | 기절 상태이다 | 기절 |
| `confused` | 혼란 상태이다 | 혼란 |
| `hallucinating` | 환각 상태이다 | 환각 |
| `telepathic` | 텔레파시 능력이 있다 | 텔레파시 |
| `clairvoyant` | 천리안 능력이 있다 | 천리안 |
| `invisible` | 투명 상태이다 | 투명 |
| `displaced` | 변위 상태이다 | 변위 |
| `stealthy` | 은밀한 상태이다 | 은밀 |
| `invulnerable` | 무적 상태이다 | 무적 |
| `very fast` | 매우 빠른 상태이다 | 매우 빠른 |
| `fast` | 빠른 상태이다 | 빠른 |

#### C_() 사용 규칙

1. **같은 msgid가 다른 파일/컨텍스트에서 다른 번역이 필요할 때만** 사용
2. 컨텍스트 이름은 소스 파일 또는 기능을 나타냄 (예: `"insight"`, `"combat"`)
3. ko_manual.po에 반드시 `msgctxt` 행을 포함해야 함
4. 기존 `_()` 항목은 가장 일반적인 용법으로 유지

---

## 6. 번역 파일 관리

> **⚠️ LLM 에이전트 주의사항**: 번역 수정 시 **반드시 ko_manual.po**를 편집하세요!
> ko.po를 직접 수정하면 `make update-po` 실행 시 덮어씌워져 모든 작업이 손실됩니다.
> ko_manual.po에 추가한 번역은 병합 시 ko.po보다 우선 적용됩니다.

### ⚠️ 6.1 핵심 정책: ko_manual.po를 편집하세요!

```
┌─────────────────────────────────────────────────────────────┐
│  파일 구조 및 역할                                          │
├─────────────────────────────────────────────────────────────┤
│  ko_manual.po  →  ⭐ 수동 번역 (이 파일을 편집!)            │
│  ko.po         →  ❌ 자동 추출 (편집 금지!)                 │
│  ko_merged.po  →  병합 결과 (자동 생성)                     │
│  ko.mo         →  컴파일된 바이너리 (자동 생성)             │
└─────────────────────────────────────────────────────────────┘
```

| 파일 | 편집 | update-po 영향 | 설명 |
|------|------|----------------|------|
| `ko_manual.po` | ⭐ O | 없음 | 안전! 항상 이 파일 편집 |
| `ko.po` | ❌ X | 덮어쓰기 가능 | 위험! 편집 비권장 |

### 6.2 왜 ko.po를 직접 편집하면 안 되나요?

1. **덮어쓰기 위험**: `make update-po` 실행 시 소스에서 다시 추출
2. **데이터 손실**: 구조 변경 시 수동 편집 내용 손실 가능
3. **병합 충돌**: ko_manual.po와 중복 시 혼란 발생

### 6.3 병합 우선순위

```
ko_manual.po (우선) + ko.po (보조) → ko_merged.po
```

**동일한 msgid가 있으면 ko_manual.po의 번역이 사용됩니다.**

#### ⚠️ 빈 문자열 제한사항

`msgcat --use-first`는 빈 문자열(`msgstr ""`)을 "미번역"으로 간주하여 무시합니다.
따라서 ko_manual.po에서 빈 문자열로 덮어쓰기가 불가능합니다.

**빈 문자열이 필요한 경우:**
1. ko.po를 직접 수정 (update-po 시 주의 필요)
2. 또는 조합 메시지의 다른 부분에서 의미를 완성하도록 번역

예: 동사 접두어 `"are "`, `"were "`는 ko.po에서 이미 빈 문자열로 되어 있음.

### 6.4 안전한 워크플로우

#### 일반 번역 작업

```bash
cd po

# 1. ko_manual.po 편집
vi ko_manual.po

# 2. 병합 + 컴파일 + 설치
make compile && make install DESTDIR=../dat
```

#### 소스에서 새 문자열 추출

```bash
cd po

# 1. POT 템플릿 생성
make pot

# 2. 안전하게 ko.po 업데이트 (백업 자동 생성)
make safe-update

# 3. 새 문자열을 ko_manual.po에 번역 추가
vi ko_manual.po

# 4. 병합 + 컴파일
make compile
```

### 6.5 ko_manual.po에 추가할 항목들

- 몬스터 이름 (`newt` → `도롱뇽`)
- 아이템 이름 (`long sword` → `장검`)
- 역할/종족 이름
- **포맷 문자열 어순 수정** (위치 지정자 `%1$s`, `%2$s` 사용)
- **조합 메시지 어순 수정** (from_what() 관련 등)
- **ko.po의 fuzzy/잘못된 번역 덮어쓰기**
- ko.po의 잘못된 번역 수정 (덮어쓰기)
- 자동 추출되지 않는 문자열

### 6.6 PO 파일 구조

```
# 주석
#: src/file.c:123        # 소스 위치
#, c-format              # 포맷 플래그
msgid "English"          # 원문 (영어)
msgstr "한국어"          # 번역
```

### 6.7 상태 확인

```bash
# 전체 통계
make stats

# 미번역 목록
make untranslated

# fuzzy 목록
make fuzzy
```

---

## 7. 조합 메시지 패턴 번역

### 7.1 문제 상황

`enl_msg()` 함수가 문자열을 조합:

```c
enl_msg(prefix, present_verb, past_verb, suffix, ps);
// 결과: prefix + verb + suffix + ps
```

영어와 한국어 어순이 달라서 문제 발생.

### 7.2 해결 전략

#### 전략 1: 명사화 패턴 (권장)

```
영어: "You hunger rapidly"
한국어: 당신은 + "배고픔이" + " 빠르게 증가합니다"
결과: "당신은 배고픔이 빠르게 증가합니다"
```

#### 전략 2: 상태 표시 패턴

```
영어: "You fall asleep uncontrollably"
한국어: 당신은 + "잠듦:" + " 통제 불가"
결과: "당신은 잠듦: 통제 불가"
```

#### 전략 3: 의미 재배치

```
영어: "You have gone without food"
한국어: verb="음식 없이" suffix=" 지냈습니다"
결과: "당신은 음식 없이 지냈습니다"
```

### 7.3 주의사항

- 번역을 한쪽으로 합치지 말 것
- 각 부분에 의미 분산 유지
- from_what() 추가 고려

### 7.4 조합 메시지 작업 체크리스트 (필수)

PO에서 `msgstr`만 읽고 예쁘면 **실패**할 수 있다. **소스에서 이어 붙인 뒤 게임에 한 줄로 나올 문장**을 먼저 적어 본다.

#### 1) 조합인지 판별

| 신호 | 예 |
|------|-----|
| `enl_msg` / `you_have` / `you_can` | `insight.c` enlightenment |
| `Sprintf` + 여러 `_()` 인자 | `trap.c` 지뢰·함정, `eat.c` 곰 덫 |
| `msgid`가 문장 일부만 | `"There is"`, `"disarm"`, `"You discover"` |
| `msgctxt` | `trap_action`, `bear_trap_eat`, `You_hear` |

```bash
# 저장소 루트 — msgid·영문 주변 검색
rg -n 'msgid_here|in a pile of soil|enl_msg' src
```

#### 2) 시뮬레이션 (수정 전·후)

1. 소스에서 **포맷 문자열**과 **인자 순서** 확인 (`%1$s` …).
2. 각 조각의 `msgstr`을 넣어 **최종 한 줄**을 손으로 적는다.
3. 분기(이미 봤음 / 자작 지뢰 / 과거형 등)마다 한 줄씩.
4. 어색하면 **포맷 재배치** 또는 **`msgctxt` 분리**(소스·PO 동시) 검토.

#### 3) 자주 하는 실수

| 실수 | 결과 | 대응 |
|------|------|------|
| 같은 영문 `msgid`를 다른 조합에 재사용 | `disarm` → 「곰 덫은 해제하기 수밖에…」 | `C_("trap_action",…)` / `C_("bear_trap_eat",…)` |
| `There is` / `There is ` 혼동 | 지뢰·「여기에」 문맥 꼬임 | 주석·msgid 공백 그대로 유지 |
| `enl_msg` 접두만 짧게 | enlightenment 「재생한다 (원인)」 주어 누락 | 접두에 **완전한 절** (`당신은 재생할 수 있다`) |
| `enl_msg` 접미 `" conflict"` 등 | 영어 조각이 한글에 노출 | 해당 `msgstr`는 **ZWSP(​)** — 접두에 의미 포함 |
| 조각에 `{이/가}` | 「발견했다{이/가} …」 | 조각은 **주어·서술어 형태**로 통일 |

#### 4) PO 주석 규칙 (`ko_manual.po`)

- 조합 블록은 `# [조합]` 또는 `# [조합 조각]`으로 시작.
- **소스 위치** (`trap.c:2565`), **호출 형태**, **시뮬 결과 2~4줄**을 적는다.
- 같은 영문이 다른 `msgctxt`이면 **「혼동 금지」**를 명시 (`disarm` 등).
- 대표 사례는 파일 내 주석과 동기화: 지뢰·흙더미, `trap_action`/`bear_trap_eat`, `You regenerate`/`You cause`.

#### 5) 검증

- `cd po && msgfmt -c -o /dev/null ko_manual.po`
- enlightenment·함정·상점 등 **해당 동작**을 게임에서 재현 (가능할 때)
- PR 전: [TRANSLATION_PROCESS.md](TRANSLATION_PROCESS.md) §2.D 표 + `./scripts/translation-preflight.sh`

운영 절차(다듬기·PR)는 [TRANSLATION_PROCESS.md](TRANSLATION_PROCESS.md) §2.D를 본다.

---

## 8. from_what() 원인 표시 처리

### 8.1 반환값 목록

| 영어 | 한국어 |
|------|--------|
| `" because of %s"` | `" (%s의 영향)"` |
| `" from birth"` | `" (선천적)"` |
| `" innately"` | `" (타고난 능력)"` |
| `" intrinsically"` | `" (내재적 능력)"` |
| `" because of your experience"` | `" (경험으로 획득)"` |
| `" due to your lycanthropy"` | `" (수인화의 영향)"` |
| `" from your creature form"` | `" (현재 형태의 영향)"` |

### 8.2 괄호 형식 이유

```
영어: "You hunger rapidly because of the ring"
한국어: "당신은 배고픔이 빠르게 증가합니다 (반지의 영향)"
```

괄호 형식은 어순과 무관하게 자연스러움.

---

## 9. 동사 접두어 상수 처리

### 9.1 목록

```c
#define are         _("are ")
#define were        _("were ")
#define have        _("have ")
#define had         _("had ")
#define can         _("can ")
#define could       _("could ")
```

### 9.2 처리 방법

**제로 폭 공백(Zero-Width Space, U+200B)으로 번역**:

```
msgid "are "
msgstr "​"   # ← 제로 폭 공백 (눈에 안 보임)
```

**중요**: 빈 문자열(`""`)은 gettext에서 영어 원문으로 폴백됨!
반드시 제로 폭 공백(​)을 사용해야 함.

이유: 한국어는 동사가 문장 끝에 옴.
의미는 suffix에서 완성.

### 9.3 동사 접미사 처리

영어 복수형/시제 접미사도 제로 폭 공백 사용:

| 접미사 | 용도 | 번역 |
|--------|------|------|
| `"s"` | 복수형/3인칭 단수 (skills, contains) | `"​"` |
| `"ed"` | 과거형 (prayed, contained) | `"​"` |
| `"es"` | 복수형 (wishes) | `"​"` |
| `"ing"` | 진행형/명사형 (gleaming) | `"기"` |

**주의**: `"ing"` → `"기"`는 특별 처리 (빛나 + 기 = 빛나기)

### 9.4 주의

제로 폭 공백이 **정상**! 반드시 주석 필요.

---

## 10. 주석 작성 규칙

### 10.1 조합 메시지 주석

```
# ============================================================
# [조합 메시지] insight.c:1170 - "You hunger/hungered rapidly"
# 영어: You_ + "hunger"/"hungered" + " rapidly" + from_what()
# 한국어: 당신은 + "배고픔이" + " 빠르게 증가합니다" + [원인]
# from_what(): "" / " (선천적)" / " (%s의 영향)" 등
# 결과: "당신은 배고픔이 빠르게 증가합니다 (반지의 영향)"
# 주의: 개별 문자열만 보면 오역 같지만 조합하면 자연스러움
# ============================================================
```

### 10.2 동사 접두어 주석

```
# ============================================================
# [조합 메시지] 동사 접두어 - be동사 (are/were/was)
# insight.c - you_are() 매크로에서 사용
# 한국어는 동사가 끝에 오므로 제로 폭 공백 사용
# 예: "You are swimming" → "당신은 수영 중"
# 주의: 빈 문자열은 영어로 폴백됨! 제로 폭 공백 필수!
# ============================================================
```

### 10.3 공유 문자열 주석

```
# 주의: 여러 곳에서 사용됨
# - insight.c:1652 (stealth)
# - insight.c:1884 (다른 컨텍스트)
# 두 컨텍스트 모두에서 작동해야 함
```

---

## 11. 특수 번역 항목

### 11.1 펫 이름 (dog.c)

```c
// src/dog.c
petname = _("Slasher");   // 베기꾼
petname = _("Hachi");     // 하치
petname = _("Idefix");    // 이데픽스
petname = _("Sirius");    // 시리우스
```

### 11.2 몬스터 이름

대부분 `monst.c`의 `mons[]` 배열에 정의.
`obj_descr_i18n.c`에서 번역 처리.

### 11.3 아이템 이름

`objects.c`의 `objects[]` 배열에 정의.
`obj_descr_i18n.c`에서 번역 처리.

### 11.4 직업/종족 이름

```
# 직업
Archeologist → 고고학자
Barbarian → 야만인
Caveman → 원시인
...

# 종족
Human → 인간
Elf → 엘프
Dwarf → 드워프
...
```

---

## 12. 도움말 및 데이터 파일 번역

### 12.1 도움말 파일 위치

```
dat/locale/ko/
├── help           # 일반 도움말
├── hh             # 짧은 도움말
├── cmdhelp        # 명령어 도움말
├── keyhelp        # 키 도움말
├── opthelp        # 옵션 도움말
├── optmenu        # 옵션 메뉴
├── wizhelp        # 마법사 도움말
├── usagehlp       # 사용법
└── history        # 역사
```

### 12.2 데이터 파일

```
dat/locale/ko/
├── rumors.tru     # 진짜 소문
├── rumors.fal     # 가짜 소문
├── epitaph.txt    # 묘비명
├── engrave.txt    # 새겨진 글
├── bogusmon.txt   # 가짜 몬스터
└── oracles.txt    # 신탁
```

### 12.3 Lua 파일

```
dat/locale/ko/*.lua
```

퀘스트 대화, 던전 설명 등.

### 12.4 데이터 파일 형식

#### rumors (소문)
```
진짜 소문 내용
%
가짜 소문은 rumors.fal에
```

#### epitaph (묘비)
```
여기 용감한 모험가가 잠들다
%
그는 용처럼 살았다
```

### 12.5 도움말 파일 번역 프로세스

#### 작업 흐름

```
1. 원본 영어 파일 확인
   cat dat/help      # 원본 영어 도움말

2. 한국어 번역 파일 생성/편집
   vi dat/locale/ko/help

3. 빌드 후 적용 확인
   cd . && make
   ./src/nethack     # ? 키로 도움말 확인
```

#### 번역 규칙

1. **형식 유지**: 원본의 줄바꿈, 공백, 정렬 유지
2. **문체**: '-다' 체 사용 (해요체 X)
3. **명령어**: 영어 명령어는 그대로 유지
   ```
   원본: Press 'h' for help
   번역: 도움말을 보려면 'h'를 누르다
   ```
4. **키 설명**: 키 이름은 영어 유지
   ```
   원본: ESC - cancel
   번역: ESC - 취소
   ```

#### 파일별 주의사항

| 파일 | 주의사항 |
|------|---------|
| help | 일반 도움말, 초보자용 설명 |
| hh | 짧은 도움말, 한 화면에 맞춰야 함 |
| cmdhelp | 명령어 설명, 정렬 유지 중요 |
| keyhelp | 키 바인딩, 포맷 유지 |
| opthelp | 옵션 설명, 기술 용어 주의 |
| wizhelp | 마법사 모드 전용 |

#### 테스트 방법

```bash
# 도움말 테스트
./src/nethack
# 게임 내에서 ? 키 → 도움말 메뉴

# 명령어 도움말 테스트
# 게임 내에서 ?i, ?c 등

# 옵션 도움말 테스트
# 게임 내에서 O (옵션 메뉴)
```

### 12.6 데이터 파일 번역 프로세스

#### 작업 흐름

```
1. 원본 데이터 파일 확인
   cat dat/rumors.tru

2. 한국어 번역 파일 생성/편집
   vi dat/locale/ko/rumors.tru

3. 빌드 & 데이터 재생성
   cd . && make

4. 게임에서 확인
   ./src/nethack    # 쿠키 읽기 등으로 확인
```

#### 형식 규칙

**rumors (소문)**
- 각 소문은 한 줄
- `%` 로 항목 구분
- `.tru`: 사실인 소문 (게임 힌트)
- `.fal`: 거짓 소문 (유머/농담)

**epitaph (묘비)**
- 각 묘비문은 한 줄
- `%` 로 항목 구분
- 게임 분위기에 맞는 문구

**engrave (새겨진 글)**
- 바닥에 새겨진 메시지
- 간결하고 신비로운 느낌

#### 번역 팁

1. **소문 번역**: 게임 힌트는 정확하게, 유머는 한국적으로 변환 가능
2. **묘비 번역**: 짧고 인상적인 문구, 한국 정서에 맞게 의역 가능
3. **수량 맞추기**: 원본과 동일한 수의 항목 유지

### 12.7 Lua 파일 번역 프로세스

#### 작업 흐름

```
1. 원본 Lua 파일 확인
   cat dat/quest.lua

2. 한국어 버전 생성/편집
   vi dat/locale/ko/quest.lua

3. 빌드 & 확인
   cd . && make
```

#### Lua 번역 규칙

```lua
-- 원본
des.message("Welcome to the Dungeons of Doom!")

-- 번역
des.message("운명의 던전에 오신 것을 환영합니다!")
```

1. **문자열만 번역**: 코드 구조는 유지
2. **이스케이프 문자**: `\n`, `\"` 등 유지
3. **변수 참조**: `%s`, `${name}` 등 유지

---

## 13. 빌드 및 검증

### 13.1 전체 빌드

```bash
# 1. 소스 빌드
cd .
make -j4

# 2. POT 생성
cd po && make pot

# 3. PO 업데이트
msgmerge -U ko.po nethack.pot

# 4. fuzzy 처리
grep -c "^#, fuzzy" ko.po

# 5. MO 컴파일
msgfmt -c -v -o messages.mo ko.po

# 6. 데이터 재생성
cd .. && make
```

### 13.2 번역만 업데이트

```bash
cd ./po
msgfmt -c -v -o messages.mo ko.po
cd .. && make
```

### 13.3 검증 체크리스트

- [ ] msgfmt 오류 없음
- [ ] fuzzy 0개
- [ ] 미번역 확인 (의도적인 것만)
- [ ] 포맷 지정자 일치
- [ ] 게임 실행 테스트

---

## 14. 테스트 방법

### 14.1 게임 실행

```bash
./src/nethack
```

### 14.2 wizard 모드

```bash
./src/nethack -D
```

테스트:
- `Ctrl+X`: 속성 확인
- `#conduct`: 행동 규칙
- `#wish`: 아이템 획득
- `#levelchange`: 레벨 변경

### 14.3 특정 메시지 테스트

1. wizard 모드 진입
2. 해당 상황 재현
3. 메시지 확인

---

## 15. 문제 해결

### 15.1 포맷 지정자 불일치

```
ko.po:1234: format specifications not same
```

해결: %s, %d 순서/개수 일치

### 15.2 인코딩 오류

```
invalid multibyte sequence
```

해결: UTF-8 확인

### 15.3 번역 미반영

1. MO 재컴파일
2. nhdat 재생성
3. 게임 재시작

### 15.4 공유 문자열 충돌

해결:
1. `C_("context", "text")`로 컨텍스트 분리 (권장, 5.8절 참조)
2. 두 컨텍스트 모두 작동하는 번역

---

## 16. 전체 작업 흐름

### 16.1 새 문자열 번역

```
1. 미번역 문자열 찾기
   msgattrib --untranslated ko.po

2. 소스 확인 (컨텍스트 파악)
   grep -n "문자열" ../src/*.c

3. 번역 추가
   에디터로 ko.po 편집

4. 컴파일 & 테스트
   msgfmt -c -v -o messages.mo ko.po
```

### 16.2 소스 코드 수정 (새 문자열 래핑)

```
1. 래핑 필요한 문자열 찾기
   grep 'pline("' ../src/file.c | grep -v '_('

2. _() 매크로로 래핑
   에디터로 소스 수정

3. 빌드
   cd .. && make -j4

4. POT 재생성
   cd po && make pot

5. PO 업데이트
   msgmerge -U ko.po nethack.pot

6. 새 문자열 번역
   에디터로 ko.po 편집

7. MO 컴파일 & 테스트
   msgfmt -c -v -o messages.mo ko.po
```

### 16.3 조합 메시지 작업

```
1. enl_msg 패턴 찾기
   grep "enl_msg" ../src/insight.c

2. _() 래핑 확인/추가

3. 패턴 분석
   - prefix, verb, suffix 조합 확인
   - from_what() 여부 확인

4. 번역 전략 결정
   - 명사화 / 상태표시 / 의미재배치

5. 번역 추가 + 주석

6. 테스트
   wizard 모드에서 해당 상황 재현
```

---

## 17. 유용한 명령어 모음

### 17.1 번역 도구 (translate-tool.sh)

번역 관리를 위한 통합 도구:

```bash
# 번역 통계
./translate-tool.sh stats

# 문자열 검색
./translate-tool.sh search "검색어"

# 조합 패턴 확인
./translate-tool.sh combo

# 조합 패턴 주석 검사
./translate-tool.sh combo-check

# 동사 접두어 번역 확인
./translate-tool.sh verb-prefix

# from_what() 패턴 확인
./translate-tool.sh from-what

# 잠재적 오류 검색
./translate-tool.sh wrong

# 번역 형식 검증
./translate-tool.sh validate

# 조사 패턴 검사
./translate-tool.sh postpos-check

# 번역 빌드 (merge + compile)
./translate-tool.sh build
```

### 17.2 검색

```bash
# 소스 검색
grep -rn "문자열" ../src/

# enl_msg 패턴
grep "enl_msg" ../src/insight.c

# 래핑 안 된 문자열
grep 'pline("' ../src/*.c | grep -v '_('

# PO 검색
grep -A2 'msgid "문자열"' ko.po
```

### 17.3 통계

```bash
# 전체 상태
msgfmt -c -v -o /dev/null ko.po

# fuzzy 개수
grep -c "^#, fuzzy" ko.po

# 미번역 개수
msgattrib --untranslated ko.po | grep -c "^msgid"

# 총 문자열
grep -c "^msgid " ko.po
```

### 17.4 편집

```bash
# PO 업데이트
msgmerge -U ko.po nethack.pot

# MO 컴파일
msgfmt -c -v -o messages.mo ko.po

# 미번역 추출
msgattrib --untranslated ko.po > untranslated.po

# fuzzy 추출
msgattrib --only-fuzzy ko.po > fuzzy.po
```

### 17.5 빌드

```bash
# POT 생성
cd po && make pot

# 전체 빌드
cd . && make -j4

# 클린 빌드
make clean && make -j4
```

---

## 부록 A: 완성된 번역 예시

### A.1 조합 메시지

```
# ============================================================
# [조합 메시지] insight.c:2126 - "You have gone/went without food"
# 영어: You_ + "have gone"/"went" + " without food"
# 한국어: 당신은 + "음식 없이" + " 지냈습니다"
# 결과: "당신은 음식 없이 지냈습니다"
# ============================================================
#: ../src/insight.c:2126
msgid "have gone"
msgstr "음식 없이"

#: ../src/insight.c:2126
msgid "went"
msgstr "음식 없이"

#: ../src/insight.c:2126
msgid " without food"
msgstr " 지냈습니다"
```

### A.2 from_what()

```
# ============================================================
# [from_what()] 원인 표시 - 괄호 형식
# ============================================================
#: ../src/attrib.c:912
#, c-format
msgid " because of %s"
msgstr " (%s의 영향)"

#: ../src/attrib.c:935
msgid " from birth"
msgstr " (선천적)"
```

### A.3 동사 접두어

```
# ============================================================
# [동사 접두어] 빈 문자열이 정상
# ============================================================
#: ../src/insight.c:44
msgid "are "
msgstr ""
```

---

## 부록 B: 자주 묻는 질문

### Q: 빈 문자열 번역이 오류인가요?
A: 동사 접두어(are, have 등)는 빈 문자열이 정상입니다.
   한국어는 동사가 문장 끝에 오기 때문입니다.

### Q: fuzzy가 뭔가요?
A: 원문이 변경되어 번역 검토가 필요하다는 표시입니다.
   확인 후 `#, fuzzy` 줄을 삭제하세요.

### Q: 공유 문자열은 어떻게 처리하나요?
A: `#:` 줄에서 여러 파일이 나열되면 공유 문자열입니다.
   `C_("context", "text")`로 컨텍스트를 분리하세요 (5.8절 참조).
   또는 모든 컨텍스트에서 작동하는 번역을 선택하세요.

### Q: 조사({은/는})가 안 되는데요?
A: `ko_postpos.c`가 제대로 링크되었는지 확인하세요.
   `#include "i18n.h"`도 필요합니다.

---

*이 가이드는 HanNetHack 번역 작업의 모든 내용을 담고 있습니다.*
*다음 세션에서 이 파일을 참조하면 바로 작업을 시작할 수 있습니다.*
