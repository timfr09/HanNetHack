...
function mk_dgl_extrainfo()
    if ((prev_dgl_extrainfo == 0) or (prev_dgl_extrainfo + 50 < u.moves)) then
        ...
        if extrai then
            ...
            nh.pline("dgl extrainfo 파일을 열 수 없음: " .. err);
        end
...
function show_getpos_tip()
   nh.text([[
팁: 원거리 보기 또는 지도 위치 선택

현재 "원거리 보기" 모드입니다 - 이동 키가 캐릭터가 아닌 커서를
움직입니다. 게임 시간은 흐르지 않습니다. 이 모드는 지도를 둘러보거나
지도에서 위치를 선택하는 데 사용됩니다.

이 모드에서 ESC를 누르면 일반 게임 모드로 돌아가고,
?를 누르면 키 도움말이 표시됩니다.
]]);
end
